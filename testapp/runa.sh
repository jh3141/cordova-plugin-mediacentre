...
if [ -d platforms/android/cordova/plugins/uk.org.dsf.cordova.media ]; then
    echo -e "${GRN}Plugman uninstall...${WHT}"
    $NODEEXEC plugman uninstall --platform android --project "platforms/android" --plugin "uk.org.dsf.cordova.media" || exit 1
    echo -e "${GRN}OK"
fi

echo -e "${GRN}Plugman install...${WHT}"
$NODEEXEC plugman install --platform android --project "platforms/android" --plugin "../mediacentre" || exit 1
echo -e "${GRN}OK"
